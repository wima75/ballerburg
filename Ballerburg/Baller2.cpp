...
int comp()  /* F��hrt einen Zug des Computers durch */
{
   char wd;
   int i,t;
...
void schuss( short k ) {
    ...
    // Dies funtkioniert hier nicht. Um den Code trotzdem möglichst 1:1 übernehmen zu können,
    // habe ich dies mit diesen komischen Callbacks und Goto's gelöst. Recht hässlich, aber funktioniert ;-)
...
void zahl( short nr, short wert ) /* 5-stellige Zahl, rechtsb�ndig, ohne f�hrende Nullen */
{
   short i,a,b;
   char *adr;
...
char kna[]="[0][Der K�nig meint:             |'",
     ...
     kn5[]="Vielleicht mal 'nen| F�rderturm kaufen...",
    ...
    kn10[]="Sch�n, Sie zu sehen...",
    kn11[]="Was soll ich denn in| so einer fr�hen Phase| schon sagen?",
    kn12[]="Sie sollten mehr Geld| verdienen, F�rdert�rme bauen| und den Gegner besiegen.",
...
void koenig()
{
   ...
   if( !(Random()%20) || kn[n]&16 || (kn[n]&15)>9 )
      form_alert(1,"[0][Der K�nig hat keine Lust,   |dich zu sprechen.][Schade]");
   else
   {
